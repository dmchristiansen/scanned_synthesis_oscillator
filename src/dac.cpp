...
void DACInterface::Init(volatile uint16_t* source) {
	...
	DMA_TCD0_CITER_ELINKNO = DMA_TCD_CITER_ELINKYES_CITER((OUTPUT_BUFFER_SIZE*OUTPUT_BUFFER_COUNT) / 8);
	DMA_TCD0_BITER_ELINKNO = DMA_TCD_BITER_ELINKYES_BITER((OUTPUT_BUFFER_SIZE*OUTPUT_BUFFER_COUNT) / 8);
	DMA_TCD0_CITER_ELINKNO = ((OUTPUT_BUFFER_SIZE*OUTPUT_BUFFER_COUNT) / 8)	& 0x7FFF;
	DMA_TCD0_BITER_ELINKNO = ((OUTPUT_BUFFER_SIZE*OUTPUT_BUFFER_COUNT) / 8) & 0x7FFF;

	DMA_TCD0_SLAST = -((int16_t)(OUTPUT_BUFFER_SIZE*OUTPUT_BUFFER_COUNT) *2); // *
	DMA_TCD0_DLASTSGA = 0; // *
